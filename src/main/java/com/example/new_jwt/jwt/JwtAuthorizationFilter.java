...
public class JwtAuthorizationFilter extends BasicAuthenticationFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
        // 아래 super를 지우지 않으면 filter가 두번거쳐진다
//        super.doFilterInternal(request, response, chain);
        System.out.println("인증이나 권한이 필요한 주소 요청 입니다.");
        ...
        //JWT토큰을 검증해서 정상적인 사용자인지 확인해봐야 한다.
        String jwtToken = request.getHeader("Authorization").replace("Bearer ", "");

        // username이 들어오면 서명이 된 토큰
        String username =
        ...
        if (username != null) {
            ...
            // username이 null 이 아니기 때문에 authentication 객체를 새로 만들어줄 수 있는 것이다.
            // username이 null이 아니라는 것은 유저가 정상적으로 인증이 됐다는 것이다.
